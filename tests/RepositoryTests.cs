...
public class RepositoryTests
{
    ...
    [Test]
    public void RepositoryCanAttackNewCharactersAndOnModifierChangesArmorIsModified()
    { 
        var dbContextOptions = new DbContextOptionsBuilder<EvercraftDbContext>()
            .UseInMemoryDatabase("ModifierChangesHitStrengthModifierDoublesHit").Options;
        var evercraftDbContext = new EvercraftDbContext(dbContextOptions);
        var homeRepository = new HomeRepository(
            evercraftDbContext);
        homeRepository.CreateCharacter("can attack character");
        homeRepository.SetModifier(1, 10, "Dexterity");

        homeRepository.AttackCharacter(1, 13);
